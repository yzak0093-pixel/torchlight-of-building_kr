...
export const getBaseAffixes = (rarity: PrismRarity): PrismAffix[] => {
  ...
  return Prisms.filter((p) => {
    ...
    const isEngLeg =
      p.affix.startsWith("Replaces") || p.affix === PHANTASMAGORIA_AFFIX;
    const isKoLeg =
      p.affix.includes("핵심 재능 바꾸기") ||
      p.affix.includes("랜덤 옵션의 효과");
...
export const getAreaAffixes = (): PrismAffix[] => {
  ...
  return Prisms.filter((p) => {
    const isEngArea =
      p.type === "Random Affix" &&
      p.affix.startsWith("The Effect Area expands to");
    const isKoArea =
      p.type === "Prism Area" ||
      p.affix.includes("범위가") ||
      p.affix.includes("영역");
...
export const getMutationAffixes = (): PrismAffix[] => {
  ...
  return Prisms.filter((p) => {
    const isEngMut =
      p.type === "Random Affix" && p.affix.includes("Mutated Core Talents");
...
export const getBaseAffixLabel = (baseAffix: string): string => {
  if (
    baseAffix === PHANTASMAGORIA_AFFIX ||
    baseAffix.includes("랜덤 옵션의 효과 +75%")
  )
    return "Phantasmagoria (랜덤 옵션 효과 +75%)";
