...
const main = async () => {
  const contextPath = join(
    ...
    "components",
    "builder",
    "OffenseResultsContext.tsx",
  );

  // 파일 통째로 안전하게 덮어씌우기
  const newContent = `import { createContext, type ReactNode, useContext, useEffect, useMemo } from "react";
...
export const OffenseResultsProvider = ({
  ...
  const equippedItems = useMemo(() => {
    ...
    for (const key in slots) {
      ...
      if (item && typeof item === "object" && (item.implicits || item.affixes)) {
        items.push(item);
      }
    }
    return items;
...
`;
  ...
  console.log("✅ OffenseResultsContext.tsx에 한글 스탯 엔진 완벽 이식 완료!");

  try {
    console.log("🎨 포맷팅 적용 중...");
    execSync("pnpm format", { stdio: "inherit" });
    console.log("🎉 시스템 통합 성공! 브라우저를 확인하세요.");
  } catch (e) {
